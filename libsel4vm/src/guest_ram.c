...
{
    /***
     * Why is it promising ? (FIXME): please make sure we understand
     * the meaning of what 'map_one_on_one' is in 'vm_config', which
     * was generated by CAmkES.
     */
    if (!vm->mem.map_one_to_one) {
        return vm_memory_try_mapping_deferred_pages(vm, start, bytes, ram_alloc_iterator, (void *)vm);
    }
    /* stage-1 physical address is one_to_one to stage-2 physical address */
    return vm_memory_try_mapping_deferred_pages(vm, start, bytes, ram_ut_alloc_iterator, (void *)vm);
}
